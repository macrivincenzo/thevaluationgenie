...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/comparisons", requireSimpleAuth, async (req, res) => {
    try {
  ...
  app.get("/api/comparisons", requireSimpleAuth, async (req, res) => {
    try {
      const userId = (req as any).user.id;
  ...
  // Get specific comparison with items
  app.get("/api/comparisons/:id", requireSimpleAuth, async (req, res) => {
    try {
      const { id } = req.params;
      const userId = (req as any).user.id;
      ...
      const items = await storage.getComparisonItems(id);
  ...
  // Update comparison
  app.patch("/api/comparisons/:id", requireSimpleAuth, async (req, res) => {
    try {
      const { id } = req.params;
      const userId = (req as any).user.id;
      ...
      const updatedComparison = await storage.updateComparison(id, updates);
  ...
  // Delete comparison
  app.delete("/api/comparisons/:id", requireSimpleAuth, async (req, res) => {
    try {
      const { id } = req.params;
      const userId = (req as any).user.id;
  ...
  // Add valuation to comparison
  app.post("/api/comparisons/:id/items", requireSimpleAuth, async (req, res) => {
    try {
      ...
      const validatedData = insertComparisonItemSchema.parse({
        ...req.body,
        comparisonId
      });
      
      const item = await storage.addComparisonItem(validatedData);
      res.json(item);
    } catch (error: any) {
      console.error("Error adding comparison item:", error);
      if (error.name === "ZodError") {
        return res.status(400).json({ message: "Invalid input", errors: error.errors });
      }
      res.status(500).json({ message: "Failed to add item to comparison" });
    }
  });

  // Remove valuation from comparison
  app.delete("/api/comparisons/:id/items/:valuationId", requireSimpleAuth, async (req, res) => {
    try {
      const { id: comparisonId, valuationId } = req.params;
      const userId = (req as any).user.id;
      ...
      await storage.removeComparisonItem(comparisonId, valuationId);
      res.status(204).send();
    } catch (error: any) {
      console.error("Error removing comparison item:", error);
      res.status(500).json({ message: "Failed to remove item from comparison" });
    }
  });

  // Update comparison item notes
  app.patch("/api/comparisons/items/:itemId", requireSimpleAuth, async (req, res) => {
    try {
      const { itemId } = req.params;
      const { notes } = req.body;
      
      const updatedItem = await storage.updateComparisonItemNotes(itemId, notes);
      res.json(updatedItem);
    } catch (error: any) {
      console.error("Error updating comparison item notes:", error);
